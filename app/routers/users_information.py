...
def get_user_info(db: Session = Depends(get_db)) -> CommonResponseDTO:
    try:
        return controller.get_user_information(db)
...
async def save_user_information(user_information: UserInformationDTO,
    ...
    return controller.save_user_information(user_information, db)
...
def delete_user_information(id_user_information: int, response: Response, db: Session = Depends(get_db)):
    delete_response = controller.delete_user_information(id_user_information, db)
